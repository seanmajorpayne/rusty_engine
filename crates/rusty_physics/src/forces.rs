...
// generate_drag_force for a Body
...
pub fn generate_gravitational_force(body_one: &body::Body, body_two: &body::Body, gravitational_constant: f32) -> (na::Vector2<f32>, na::Vector2<f32>) {
    let difference = body_two.position() - body_one.position();
    ...
    let attraction_force = gravitational_constant * (body_one.mass() * body_two.mass() / distance_squared) * direction;
